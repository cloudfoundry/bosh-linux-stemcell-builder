...
var _ = Describe("Stemcell", func() {
	Context("When syslog release has been deployed", func() {
		...
		Context("when logrotate wtmp/btmp logs", func() {
			It("should rotate the wtmp/btmp logs", func() {
				stdOut, stdErr, exitStatus, err := bosh.Run("ssh", "syslog_forwarder/0", `sudo bash -c "dd if=/dev/urandom count=10000 bs=1024 >> /var/log/wtmp" \
    	...
    	&& sudo sed -i "s/0,15,30,45/\*/" /etc/cron.d/logrotate`)
				...
				stdOut, stdErr, exitStatus, err = bosh.Run("ssh", "--column=stdout", "--results", "syslog_forwarder/0", "sudo du /var/log/wtmp | cut -f1")
				Expect(err).ToNot(HaveOccurred())
				...
				stdOut, stdErr, exitStatus, err = bosh.Run("ssh", "--column=stdout", "--results", "syslog_forwarder/0", "sudo du /var/log/btmp | cut -f1")
				Expect(err).ToNot(HaveOccurred())
				fileSizeInKiloBytes, err = strconv.Atoi(strings.TrimSpace(stdOut))
				Expect(err).ToNot(HaveOccurred(), "error converting kB file size to integer")
				Expect(fileSizeInKiloBytes).To(BeNumerically("<", 100), "Logfile was larger than expected. It should have been rotated.")
	...
	Context("when syslog threshold limit is reached", func() {
		It("should rotate the logs", func() {
			stdOut, stdErr, exitStatus, err := bosh.Run("ssh", "syslog_forwarder/0", `logger "old syslog content" \
	...
	&& sudo sed -i "s/0,15,30,45/\*/" /etc/cron.d/logrotate`)
			...
			stdOut, stdErr, exitStatus, err = bosh.Run("ssh", "syslog_forwarder/0", `logger "new syslog content"`)
			Expect(err).ToNot(HaveOccurred())
			Expect(exitStatus).To(Equal(0), fmt.Sprintf("Could not log using logger on the syslog forwarder! \n stdOut: %s \n stdErr: %s", stdOut, stdErr))

			stdOut, stdErr, exitStatus, err = bosh.Run("ssh", "syslog_forwarder/0", `sudo cat /var/vcap/data/root_log/syslog`)
			Expect(err).ToNot(HaveOccurred())
			Expect(exitStatus).To(Equal(0))
	...
	It("#134136191: auth.log should not contain 'No such file or directory' errors", func() {
		...
		stdOut, stdErr, exitStatus, err := bosh.Run("ssh", "syslog_forwarder/0", `sudo cp /var/log/auth.log /tmp/ && sudo chmod 777 /tmp/auth.log`)
		Expect(err).ToNot(HaveOccurred())
		Expect(exitStatus).To(Equal(0), fmt.Sprintf("Could not create nested log path! \n stdOut: %s \n stdErr: %s", stdOut, stdErr))

		stdOut, stdErr, exitStatus, err = bosh.Run("scp", "syslog_forwarder/0:/tmp/auth.log", authLogAbsPath)
		Expect(err).ToNot(HaveOccurred())
		Expect(exitStatus).To(Equal(0))
	...
	It("#133776519: forwards deeply nested logs", func() {
		stdOut, stdErr, exitStatus, err := bosh.Run("ssh", "syslog_forwarder/0", `sudo mkdir -p /var/vcap/sys/log/deep/path && sudo chmod 777 /var/vcap/sys/log/deep/path && touch /var/vcap/sys/log/deep/path/deepfile.log`)
		Expect(err).ToNot(HaveOccurred())
		Expect(exitStatus).To(Equal(0), fmt.Sprintf("Could not create nested log path! \n stdOut: %s \n stdErr: %s", stdOut, stdErr))
	...
	It("#141987897: disables ipv6 in the kernel", func() {
		stdOut, _, exitStatus, err := bosh.Run("--column=stdout", "ssh", "syslog_forwarder/0", "-r", "-c", `sudo netstat -lnp | grep sshd | awk '{ print $4 }'`)
		Expect(err).ToNot(HaveOccurred())
		Expect(exitStatus).To(Equal(0))
