...
# (docs/ci-warden-rosetta-build.md).
#
...
run_in_chroot $chroot "
  rm -f $probe_marker
  /usr/bin/tar --version >/dev/null 2>&1 && : > $probe_marker
  true
"
...
if [ -f "$chroot$probe_marker" ]; then
  ...
  echo "base_ubuntu_warden_rosetta: SKIPPED all execution-based checks (arm64" \
       "tar round-trip) — this build host cannot execute arm64 binaries. The" \
       "swapped binaries were verified statically only; run the build on Apple" \
       "Silicon to exercise them."
fi
