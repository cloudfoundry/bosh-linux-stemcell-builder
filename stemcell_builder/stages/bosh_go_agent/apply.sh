...
cd $assets_dir
if is_ppc64le; then
  curl -L -o bosh-agent "https://s3.amazonaws.com/bosh-agent-binaries/bosh-agent-2.91.2-linux-ppc64le"
  echo "ac17ce756c8aad0c4a65b2a702adf36c21e888fe009fc61cb959ea6d28ec78fe  bosh-agent" | shasum -a 256 -c -
else
  curl -L -o bosh-agent "https://s3.amazonaws.com/bosh-agent-binaries/bosh-agent-2.91.2-linux-amd64"
  echo "3160da1c38b9adc1cf50cc31f636690734648bc0f07aecfdcd13afc2f9a175f6  bosh-agent" | shasum -a 256 -c -
fi
