...
describe 'RHEL 8 stemcell', stemcell_image: true do
  ...
  describe 'installed packages' do
    ...
    let(:rpm_list_rhel) { File.readlines(spec_asset('rpm-list-rhel-8.txt')).map(&:chop) }
    let(:rpm_list_google_rhel) { File.readlines(spec_asset('rpm-list-rhel-8-google-additions.txt')).map(&:chop) }
    let(:rpm_list_vsphere_rhel) { File.readlines(spec_asset('rpm-list-rhel-8-vsphere-additions.txt')).map(&:chop) }
    let(:rpm_list_azure_rhel) { File.readlines(spec_asset('rpm-list-rhel-8-azure-additions.txt')).map(&:chop) }
    let(:rpm_list_softlayer_rhel) { File.readlines(spec_asset('rpm-list-rhel-8-softlayer-additions.txt')).map(&:chop) }
