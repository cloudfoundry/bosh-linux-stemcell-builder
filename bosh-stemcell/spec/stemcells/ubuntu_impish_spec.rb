...
HERE
  ...
  describe 'installed packages' do
    ...
    let(:dpkg_list_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-impish.txt')).map(&:chop) }
    let(:dpkg_list_google_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-impish-google-additions.txt')).map(&:chop) }
    let(:dpkg_list_vsphere_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-impish-vsphere-additions.txt')).map(&:chop) }
    let(:dpkg_list_azure_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-impish-azure-additions.txt')).map(&:chop) }
    let(:dpkg_list_cloudstack_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-impish-cloudstack-additions.txt')).map(&:chop) }
    let(:dpkg_list_softlayer_ubuntu) { File.readlines(spec_asset('dpkg-list-ubuntu-impish-softlayer-additions.txt')).map(&:chop) }
